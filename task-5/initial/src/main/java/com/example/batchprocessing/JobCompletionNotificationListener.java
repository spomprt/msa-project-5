...
public class JobCompletionNotificationListener implements JobExecutionListener {
    ...
    public void afterJob(JobExecution jobExecution) {
        if (jobExecution.getStatus() == BatchStatus.COMPLETED) {
            ...
            metricsService.incrementBatchJobCompleted();
            ...
            log.info("  Total products processed: {}", metricsService.getTotalProductsProcessed());
            log.info("  Products with loyalty updated: {}", metricsService.getTotalProductsWithLoyaltyUpdated());
            log.info("  Products with loyalty kept: {}", metricsService.getTotalProductsWithLoyaltyKept());
            
        } else {
            log.error("Job failed with status: {}", jobExecution.getStatus());
            
            // Increment job failed counter
            metricsService.incrementBatchJobFailed();
        }
    }
}
